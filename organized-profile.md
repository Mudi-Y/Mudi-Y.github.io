# Organized Profile from mudiyang.com

## Honors, prizes, awards, and fellowships
- Awards:
  - YHack Hackathon — Winter 2022
    - Top 5 finalist; Awards cited in CV: Best COVID-19 Related Hack (Citadel), Best Use of Google Cloud (Google)
  - UCLA BIG Research Excellence Award — Summer 2020

- Fellowships:
  - Yale College Dean's Research Fellowship — Summer 2020
  - Davenport College Richter Fellowship — Summer 2019
  - Yale College First-Year Summer Research Fellowship — Summer 2019

- Summer Schools and Research Programs:
  - STAQ Quantum Ideas Summer School, Duke University (Pratt School of Engineering) — Summer 2023
  - Cornell–Maryland–Max Planck Pre-Doctoral Research School (CMMRS) — Summer 2022
  - UCLA Bruins in Genomics (BIG) Research Program (NSF REU) — Summer 2020
  - Pembroke King’s Programme, University of Cambridge — Summer 2019

## Major publications with title, citation and date
- The QUATRO Application Suite: Quantum Computing for Models of Human Cognition.
  - Citation: R. P. Pothukuchi et al., including M. Yang. arXiv:2309.00597
  - Developed quantum random-walk implementations on D-Wave quantum annealing systems to model the two-alternative forced-choice decision problem.

## Teaching and work experience
- Graphics Compiler Engineer, Qualcomm (Adreno Graphics Compiler) — 2022–Present
  - Focus on OpenGL API feature development and maintenance in Qualcomm's Adreno graphics high-level compiler.
  - Summer 2021 intern: Built an AMBER-based unit-testing framework for graphics and compute shaders; implemented package extensions based on team needs.
- Undergraduate Learning Assistant, Yale Dept. of Computer Science — 2021–2022
  - Link: [arXiv:2309.00597](https://arxiv.org/abs/2309.00597)

- Exploring the Impact of Sentiment Analysis on Current Methods of Fake News Detection.
  - Citation: M. Yang, L. Flores, H. Hunma, B. Trevisan. Yale Undergraduate Research Journal 3(1), 2022
  - Implemented a BERT-based sentiment analysis module to augment fake-news detection pipelines.
  - Link: [FakeNews.pdf](https://mudiyang.com/assets/pubs/FakeNews.pdf)

- Radiation Emissions of Primordial Black Holes as Dark Matter in a Dwarf Galaxy.
  - Citation: M. Yang, K. Holley-Bockelmann, F. Munshi. Young Scientist 8:52–54, 2018
  - Modeled expected radiation signatures under the primordial black hole dark matter hypothesis.
  - Link: [Mudi-Yang.pdf](https://cdn.vanderbilt.edu/vu-wordpress-0/wp-content/uploads/sites/16/2020/12/19123539/Mudi-Yang.pdf)

## Research experience
- Student Researcher, Rubenstein & Yuen Group, Columbia University — 2024–Present
  - Compiler research: Contributing to QExpress for multiple quantum hardware platforms.
- Student Researcher, Bhattacharjee Group, Yale University — 2021–2024
  - Cognitive modeling: Quantum random-walk model for 2AFC decision problems; continuation of senior thesis work.
- Student Researcher, Gerstein Bioinformatics Lab, Yale University — 2019–2021
  - Bioinformatics: PARSE project for causal RBP and LD variants; full-stack development.
- Summer Research Intern, Jason Ernst Lab (UCLA BIG) — Summer 2020
  - Regulatory genomics: Machine-learning extension of Sharpr-MRPA statistical model.
- Research Intern, Radev Group (LILY), Yale University — Summer 2019
  - NLP: Hybrid extractive–abstractive deep learning for multi-document summarization; work with MultiNews dataset.
- Research Intern, Holley-Bockelmann Group, Vanderbilt University — (date not specified in index; appears in projects and earlier experience)
  - Astrophysics: Modeled physical consequences of the primordial black hole dark matter hypothesis using departmental compute resources.

## Other experience, including military, volunteer work, travel
- Leadership & Service:
  - Co-President, Yale Undergraduate Aerospace Association — 2020–2021
    - Led the largest on-campus engineering club; High-Altitude Balloon Team Lead (2019–2020); developed a GPS-guided parafoil system; received 2019 NASA Connecticut Space Grant.
  - Davenport Liaison, Davenport Pops Orchestra — 2018–2021 (CV notes 2019–2022)
    - Coordinated grants, performance logistics, and relations between the orchestra and Davenport College.
- Hackathon Projects:
  - YHACK 2020 — Covid Resource App
    - Android app implementing contact tracing, social distancing mapping, and a VR social distancing “ruler”.
  - HackMIT 2021 — Streetstagram
    - Django-based site for posting and discovering street art in New Haven with Google Maps integration.
- Talks:
  - TEDxNashville Talk (2018)
    - Presented on uses of supercomputers and research from the Holley-Bockelmann group.